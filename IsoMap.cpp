	...
	for (int i = 0; i < TILE_COUNT_X; i++)
	{
		for (int j = 0; j < TILE_COUNT_Y; j++)
		{
			...
			if (_isDebug)
			{
				...
				_rhombus2.x = left + RADIUS_WIDTH / 2 + 8;
...
void IsoMap::MapToolSetup()
{
	for (int i = 0; i < TILE_COUNT_X; i++)
	{
		for (int j = 0;j < TILE_COUNT_Y; j++)
		{
			...
			_tileMap[i][j].tileKind[_tileMap[i][j].index] = TILEKIND_NONE;
		}
	}
...
void IsoMap::setMap(int isoX, int isoY, bool isAdd)
{
	...
	case CTRL_ERASER:
		if (_tileMap[isoX][isoY].index == 0)
		{
			_tileMap[isoX][isoY].tileKind[_tileMap[isoX][isoY].index] = TILEKIND_NONE;
		}
		break;
	}
}
...
{
	if (frameX == -1 && frameY == -1)return TILEKIND_TERRAIN;


	if (SUBWIN->GetFrameIndex() == 0)
	{
		if (frameY >= 5)return TILEKIND_OBJECT;
		if (frameY <= 4)return TILEKIND_OBJECT;
		else return TILEKIND_TERRAIN;
	}
...
void IsoMap::Load(int loadCount)
{
...
void IsoMap::Save(int saveCount)
{
