...
/*
	...
	오픈 리스트에서 가장 비용이 적은 위치를 현재 위치로 지정(즉 F가 작은곳) --->> 가장 비용이 작은 위치가 현재 위치가 됨.
	위 과정을 반복한다.
	F값이 동일하다면 속도상의 목적으로 오픈리스트에 더 늦게 추가 된것을 선택하는 것이 빠르다.
*/
#define TILE_X 40	//가로 타일수
#define TILE_Y 40	//세로 타일 수
#define WIDTH 20		//타일 가로 길이
#define HEIGHT 20		//타일 세로길이
